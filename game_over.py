...
def infin(use_mouse, user, play_time):
    from main import cursor_grabbed, x4, y4, cap, mp_hands, mp_drawing, mp_drawing_styles, mouse_down
    global screen, clock
    pygame.init()
    screen = pygame.display.set_mode((1280, 720))
    ...
    history_text = Text(f"All time_high: {all_time_high//60}m {all_time_high%60}s", (810, 190), 50, black)
    indiv_text = Text(f"Your highest: {your_highest//60}m {your_highest%60}s", (810, 255), 50, black)
    this_time_text = Text(f"Play time: {play_time//60}m {play_time%60}s", (810, 320), 50, black)
    pygame.time.set_timer(pygame.USEREVENT, egg.fps)
    ...
    with mp_hands.Hands(
        ...
        while True:
            ...
            screen.blit(egg.image, egg.rect)
            screen.blit(history_text.text, history_text.rect)
            screen.blit(indiv_text.text, indiv_text.rect)
            screen.blit(this_time_text.text, this_time_text.rect)
            screen.blit(exit_btn.image, exit_btn.rect)
            screen.blit(restart_btn.image, restart_btn.rect)
